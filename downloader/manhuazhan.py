...
class ManhuazhanComic(ComicSource):
    ...
    def search(self, keyword):
        ...
        search_url = (
            f'{self.base_url}/index.php/search?key={keyword}&verify=1&callback=jQuery112406074091281837971_1750596681221&pcode=anys&_=1750596681224'
        )
        arr = []
        try:
        ...
        for book in book_list:
            ...
            try:
                ...
                if not comic.url.startswith('http'):
                    comic.url = (
                        self.base_url + comic.url if comic.url.startswith('/') else self.base_url + '/' + comic.url
                    )
                ...
                logger.error(
                    f'解析漫画条目时出错: {etree.tostring(book, encoding="unicode")}, 错误: {e}', exc_info=True
                )
                continue
    ...
    def info(self, url):
        ...
        try:
            ...
            logger.warning(
                f'解析元数据时出错，跳过剩余元数据项，错误: {e}', exc_info=True
            )
        ...
        for book in book_list:
            ...
            vol_list = book.xpath('div[contains(@class,"d-play-box")]/div[contains(@class,"d-player-list")]/a')
            for vol_node in vol_list:
                try:
                    ...
                    if not vol_href.startswith('http'):
                        full_vol_url = (
                            self.base_url + vol_href if vol_href.startswith('/') else self.base_url + '/' + vol_href
                        )
                    else:
                    ...
                    logger.error(
                        f"解析卷 '{vol_title if 'vol_title' in locals() else '未知卷'}' 时出错: {e}", exc_info=True
                    )
    ...
    def __parse_imgs__(self, url):
        ...
        try:
            ...
            img_urls = self.driver.execute_script('return typeof newImgs !== "undefined" ? newImgs : [];')
            if not img_urls:
                logger.warning(f'未能从页面 {url} 获取 newImgs 变量，或变量为空.')
                # 尝试查找其他可能的图片源或结构
                # 例如，直接从img标签解析
            ...
            for img_url in img_urls:
                if not img_url or not isinstance(img_url['url'], str):
                    logger.warning(f'无效的图片URL: {img_url}')
                    continue
                # 检查URL是否完整，如果不是，则拼接
                if not img_url['url'].startswith('http'):
                    ...
                    logger.debug(f"图片URL '{img_url['url']}' 不是完整URL，将直接使用，请确认是否正确.")
