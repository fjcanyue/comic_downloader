...
class ManhuazhanComic(ComicSource):
    ...
    def search(self, keyword):
        ...
        try:
            ...
            if not main_nodes:
                ...
                logger.info('页面HTML: {}', root.xpath('string()'))
                return arr
            main = main_nodes[0]
        ...
        logger.info('共 {} 条相关的结果', len(book_list))
        for book in book_list:
            ...
            try:
                ...
                logger.error(
                    '解析漫画条目时出错: {}, 错误: {}',
                    etree.tostring(book, encoding='unicode'),
                    e,
                    exc_info=True,
                )
                continue
    ...
    def _append_book_volumes(self, book, comic_book):
        ...
        for vol_node in vol_list:
            vol_title = vol_node.text.strip() if vol_node.text else ''
        ...
        try:
            ...
            for img_url in img_urls:
                if (
                    ...
                    logger.debug('解析到图片URL: {}', img_url['url'])
                else:
                    logger.warning('无效的图片URL: {}', img_url)
