            ...
            for vol_node in vol_list:
                try:
                    ...
                    if not vol_title or not vol_href:
                        logger.warning(
                            f'卷信息不完整 (缺少title或href): {etree.tostring(vol_node, encoding="unicode")}'
                        )
                        continue
                    # Maofly的href已经是完整URL
                    full_vol_url = (
                        vol_href if vol_href.startswith('http') else self.base_url + vol_href
                    )
                    comic_book.vols.append(
                        ComicVolume(vol_title.strip(), full_vol_url, comic_book.name)
                    )
    ...
    def __decode_img_data__(self, img_data_str):
        ...
        try:
            ...
            if not decoded_string:
                ...
                return []
            logger.warning(
                f'JS解码返回了非字符串类型: {type(decoded_string)}, 内容: {str(decoded_string)[:100]}...'
            )
            return []
        except Exception as e:
            logger.error(f'使用 Selenium 解码 img_data 时发生错误: {e}', exc_info=True)
            logger.debug(
                f'执行的JS代码片段(部分): {js_code[:200]}...{js_code[-200:] if len(js_code) > 400 else ""}'
            )
            return []
